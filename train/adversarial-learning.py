...
def main(cfg, exp: ExperimentManager, device=torch.device('cpu')):
    ...
    load_critic = cfg.agent.load_critic
    ...
    if Round > 1:
        ...
        if load_critic:
            att_agent.load_all(load_att_actor, load_att_critic, load_att_critic_target)
        else:
    ...
    if Round > 1 or LearningSide == 'Att':
        ...
        if load_critic:
            def_agent.load_all(load_def_actor, load_def_critic, load_def_critic_target)
        else:
...
if __name__ == "__main__":
    ...
    if args.round is not None:
        cfg.agent.round_num = args.round
    if args.side is not None:
        cfg.agent.learning_side = args.side
